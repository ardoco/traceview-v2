            ...
            <div className="flex flex-col space-y-3">
                {config && Object.keys(config).length > 0 ? (
                    Object.entries(config).map(([key, value]) => (
                        <div key={key} className="flex flex-col sm:flex-row items-start sm:items-center py-2 border-b border-gray-100 last:border-b-0">
                            <label htmlFor={`config-${key}`} className="font-medium text-gray-700 w-full sm:w-1/2 shrink-0 pr-4">
                                {formatKey(key)}:
                            </label>
