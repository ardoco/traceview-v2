    ...
    return (
        <g transform={`translate(${node.y}, ${node.x})`}
           ...
           onClick={(event) => {
               ...
               if (event.ctrlKey) {
                   ...
                   //const updatedRoot = copyTreeWithHiddenChildren(treeRoot);
        ...
        >
            <circle r={highlightedTraceLinks.some(traceLink => traceLink.codeElementId.startsWith(node.data.path!)) ?  6 : 4}
                    fill={ hovered ? "#D3D3D3" : highlightedTraceLinks.some(traceLink => traceLink.codeElementId.startsWith(node.data.path!)) ? "#fde047" : "#999"}
                    strokeWidth={10}/>
            {textSymbol()}
            <text
                dy={"0.34em"}
                x={6}//x={node.children ? 6 : -6}
                textAnchor={"start"}//textAnchor={node.children ? "start" : "end"}
                strokeLinejoin={"round"}
                paintOrder={"stroke"}
                stroke={highlightedTraceLinks.some(traceLink => traceLink.codeElementId.startsWith(node.data.path!)) ? "#fde047" :'#fff'}
                strokeWidth={2}
                fontSize={8}
            >
                {node.data.name}
