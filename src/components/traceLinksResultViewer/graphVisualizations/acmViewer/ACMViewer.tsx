...
import ACMNode from "@/components/traceLinksResultViewer/graphVisualizations/acmViewer/ACMNode";
import * as d3 from "d3";
import ACMLink from "@/components/traceLinksResultViewer/graphVisualizations/acmViewer/ACMLink";
...
export default function ACMViewer({codeModel}: ACMViewerProps) {
    ...
    // const nodeTree = tree<CodeModelUnit>().nodeSize([dx, dy])(treeRoot);
    // const nodes = nodeTree.descendants().reverse();
    ...
    if (highlightedTraceLinks.length > 0) {
        // Map each trace link to a path in the hierarchy
        for (const traceLink of highlightedTraceLinks) {
            const matchingNode = nodes.find(n =>
                traceLink.codeElementId === n.data.path ||
                traceLink.codeElementId.startsWith(n.data.path + ".") // optional, for nested match
            );
